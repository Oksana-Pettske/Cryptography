...
const uint32_t hash_const = 27052000;
...
uint32_t simon_encrypt(simon_cipher cipher, void *plain_text, void *cipher_text) {
    if (cipher.simon_config == simon_64_32) {
        return simon_encrypt_32(cipher.round_limit, cipher.key_sched, plain_text, cipher_text);
    }
    else if (cipher.simon_config <= simon_96_48) {
        return simon_encrypt_48(cipher.round_limit, cipher.key_sched, plain_text, cipher_text);
    }
    else if(cipher.simon_config <= simon_128_64) {
        return simon_encrypt_64(cipher.round_limit, cipher.key_sched, plain_text, cipher_text);
    }
    else if(cipher.simon_config <= simon_144_96) {
        return simon_encrypt_96(cipher.round_limit, cipher.key_sched, plain_text, cipher_text);
    }
    else if(cipher.simon_config <= simon_256_128) {
        return simon_encrypt_128(cipher.round_limit, cipher.key_sched, plain_text, cipher_text);
    }
    else return -1;
}

uint32_t hash_function(uint8_t i, uint32_t hash, uint64_t x_word, uint8_t word_size) {
    uint32_t tmp_hash;
    if (i == 0) {
        hash = hash_const;
    }
    tmp_hash = (shift_one & shift_eight) ^ hash ^ shift_two;
    hash = tmp_hash ^ x_word;
    return hash;
}

uint32_t simon_encrypt_32(uint8_t round_limit, uint8_t *key_sched, uint8_t *plain_text, uint8_t *cipher_text) {
    ...
    for (i = 0; i < round_limit; i++) {
        ...
        hash = hash_function(i, hash, x_word, word_size);
    }
...
uint32_t simon_encrypt_48(uint8_t round_limit, uint8_t *key_sched, uint8_t *plain_text, uint8_t *cipher_text) {
...
uint32_t simon_encrypt_64(uint8_t round_limit, uint8_t *key_sched, uint8_t *plain_text, uint8_t *cipher_text) {
...
uint32_t simon_encrypt_96(uint8_t round_limit, uint8_t *key_sched, uint8_t *plain_text, uint8_t *cipher_text) {
...
uint32_t simon_encrypt_128(uint8_t round_limit, uint8_t *key_sched, uint8_t *plain_text, uint8_t *cipher_text) {
